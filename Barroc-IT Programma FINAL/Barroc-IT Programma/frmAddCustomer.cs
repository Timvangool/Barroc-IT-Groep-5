...
namespace Barroc_IT_Programma
{
    public partial class frmAddCustomer : Form
    {
        ...
        public frmAddCustomer(string account)
        {
            formProperties();
            DBConnect();
            username = account;
            ac = new AccountAcces(username);
        ...
        private void CreateLayout()
        {
            ...
            for (int i = 0; i < lblInfo.Length; i++)
            {
                ...
                if (lblInfo[i].Text.Contains("Amount") || lblInfo[i].Text.Contains("Ledger") || lblInfo[i].Text.Contains("Credit"))
                {
                    lblInfo[i].Hide();
                    vissible = false;
                }
                if (i < (columns.Count - 2))
                {
                    ...
                    //if (!Visible)
                    //    txtbInfo[i].Location = new Point(x, y);
                    //else
                    //    txtbInfo[i].Location = new Point((x + 100), y);
                    txtbInfo[i].Location = new Point((x + 100), y);
                    this.Controls.Add(txtbInfo[i]);
                    if (txtbInfo[i].Name.Contains("Amount") || txtbInfo[i].Name.Contains("Ledger"))
                    {
                        txtbInfo[i].Visible = false;
                        vissible = false;
                    }
                }
                else
